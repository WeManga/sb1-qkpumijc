...
export function DiscountShop({ coins, onPurchase }) {
  ...
  const handleBuy = async (offer) => {
    ...
    try {
      ...
      if (fnError || !data?.ok) throw new Error(data?.error || "Erreur d'achat");
        ...
        return (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-black text-gray-900">{offer.discount_percent}% de réduction</p>
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">
                  {offer.coin_cost.toLocaleString('vi-VN')} pièces
                </p>
              </div>
            </div>
            ...
            >
              {purchasingId === offer.id ? <Loader2 size={14} className="animate-spin" /> : 'Acheter'}
            </button>
          </motion.div>
        );
      })}
        ...
        {revealedCode && (
          ...
          >
            <p className="text-[11px] text-amber-700 font-black uppercase tracking-widest">
              Réduction {revealedCode.discountPercent}% débloquée !
            </p>
            <p className="text-[10px] text-gray-500 font-medium leading-snug">
              Copiez ce code et collez-le dans "Mon Compte" → "Entrez votre code unique" pour l'appliquer.
            </p>

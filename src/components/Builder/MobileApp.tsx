...
import { Type, Palette, Music, Share2, X, Check, Loader2, ArrowLeft } from 'lucide-react';
...
export default function MobileApp({ invitation, onInvitationChange, onSave, onBack, saving }: any) {
  ...
  const navItems = [
    ...
    { id: 'share', label: 'Partage', icon: <Share2 size={22}/> }
  ];
  ...
  return (
    <div className="fixed inset-0 bg-[#F5F5F7] flex flex-col overflow-hidden h-screen w-screen font-sans antialiased text-[#1D1D1F]">
      ...
      <AnimatePresence>
        {activeTab && (
          <div className="absolute inset-0 z-[200] flex flex-col justify-end">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setActiveTab(null)} />
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 32, stiffness: 350 }} className="relative bg-white rounded-t-[3.5rem] max-h-[85vh] flex flex-col shadow-2xl overflow-hidden">
                ...
                <div className="flex justify-between items-center px-10 py-5 border-b border-gray-50">
                    <h3 className="text-[11px] font-black uppercase tracking-[0.4em] text-amber-800">Modifier {activeTab}</h3>
                    <button onClick={() => setActiveTab(null)} className="p-2 bg-gray-50 rounded-full text-gray-400"><X size={18}/></button>
                </div>
      ...
      <nav className="h-24 bg-white/90 backdrop-blur-2xl border-t border-gray-100 flex items-center justify-around px-6 pb-8 z-[180] fixed bottom-0 inset-x-0">
        {navItems.map((item) => (
          <button key={item.id} onClick={() => setActiveTab(item.id)} className={`flex flex-col items-center gap-1 flex-1 transition-all ${activeTab === item.id ? 'text-amber-600 scale-110' : 'text-gray-400'}`}>
            <div className={`p-3 rounded-[1.25rem] transition-all ${activeTab === item.id ? 'bg-amber-50 shadow-inner' : ''}`}>{item.icon}</div>

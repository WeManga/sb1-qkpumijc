      ...
      <AnimatePresence mode="wait">
        {view === 'envelope' ? (
          <motion.div key="env" className="relative w-full h-full flex items-center justify-center" style={{ perspective: '1200px' }}>
            ...
            {/* --- CONTENEUR SANDWICH GÉRANT LE FLUX DE SORTIE DE LA CARTE --- */}
            <div className="relative w-[310px] h-[370px] flex items-center justify-center">

              {/* --- LOGIQUE D'ANIMATION DES MÉCANISMES ARRIÈRE (VINYLE OU PELLICULE) --- */}
              <motion.div 
                initial={{ y: 0 }} 
                animate={isOpened ? { y: invitation.opening_type === 'filmstrip' ? -110 : -70 } : { y: 0 }} 
              ...
              >
                {invitation.opening_type === 'filmstrip' ? (
                  ...
                  <div className={`w-[250px] h-[250px] relative ${isOpened ? 'animate-disk-spin' : ''}`}>
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="w-20 h-20 bg-white rounded-full border-[5px] border-[#111] overflow-hidden">
              ...
              {/* --- CARTE CENTRALE ASSOCIEE AU MOUVEMENT DE GLISSEMENT VERTICAL --- */}
              <motion.div 
                initial={{ scale: 0.95, y: 0 }} 
                animate={isOpened ? { scale: 1, y: -45 } : { y: 0 }} 
                ...
                className={`z-20 w-full h-full rounded-[2.5rem] shadow-xl p-8 flex flex-col items-center justify-between border border-gray-100 cursor-pointer ${getPaperClass()}`}
              >
                <div className="text-center pt-8 w-full">
                  <h2 className="text-xl font-black uppercase tracking-tighter mb-3 break-words" style={{ fontFamily: invitation.font_style }}>
                    {invitation?.title || tBuilder.title_placeholder}
                  </h2>
                  <div className="w-8 h-1 bg-amber-400 mx-auto mb-3" />
                  <p className="opacity-60 text-[8px] font-bold uppercase tracking-[0.3em]">{t.tap_open}</p>
                </div>
                <div className="w-full py-3 bg-gray-900 text-white rounded-xl text-[9px] font-black uppercase text-center tracking-widest">
                  {lang === 'vi' ? 'Xem chi tiết' : lang === 'en' ? 'See details' : 'Voir les détails'}
                </div>
              </motion.div>

              {/* --- SIMULATION DU VISUEL DE FOND ET DE LA COUVERTURE AVANT (CONSTRUITE EN PUR CSS) --- */}
              <div className="absolute inset-0 pointer-events-none z-30 w-full h-full">
                {invitation.container_open === 'metal_door' ? (
                  ...
                  /* --- TEST : LE LOGEMENT DE L'ENVELOPPE GÉOMÉTRIQUE CSS --- */
                  <div className="absolute inset-0 w-full h-full relative overflow-hidden rounded-[2.5rem]">
                    {/* Fond arrière de la pochette papier */}
                    <motion.div 
                      animate={isOpened ? { opacity: 0.4 } : { opacity: 1 }}
                      className="absolute inset-0 w-full h-full transition-opacity z-[5]"
                      style={{ background: invitation?.envelope_color || '#FEE2E2' }}
                    />
                    
                    {/* Face avant géométrique (Forme de V inversé recouvrant la partie inférieure de la carte) */}
                    {!isOpened && (
                      <div className="absolute bottom-0 left-0 w-full h-[60%] z-40 overflow-hidden shadow-[0_-10px_20px_rgba(0,0,0,0.05)]" style={{ background: invitation?.envelope_color || '#FEE2E2' }}>
                        {/* Diagonale Gauche */}
                        <div className="absolute top-0 left-0 w-full h-full bg-black/5 origin-top-left -rotate-[35deg] border-t border-white/40" />
                        {/* Diagonale Droite */}
                        <div className="absolute top-0 right-0 w-full h-full bg-black/5 origin-top-right rotate-[35deg] border-t border-white/40" />
                        {/* Logo / Badge centrale pour habiller la fente de l'enveloppe */}
                        <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-sm px-4 py-1.5 rounded-full border border-gray-200/40 text-[8px] font-bold text-gray-500 uppercase tracking-widest shadow-sm">
                          Invit Studio
                        </div>
                      </div>
                    )}
            ...
            {/* --- COUCHE DECLENCHEURS INTERACTIFS --- */}
            <div className="absolute inset-0 z-50 overflow-hidden" style={{ perspective: '2000px', pointerEvents: isOpened ? 'none' : 'auto' }}>
              <div className="w-full h-full relative flex items-center justify-center">
                <AnimatePresence>
                  {!isCodeFading && (!isOpened || invitation.container_open === 'metal_door') && (
                    <motion.div 
                      key="visual-trigger"
                      initial={{ opacity: 1 }}
                    ...
                    {(invitation.event_program || []).map((step: any, i: number) => {
                      ...
                      return (
                        <motion.div key={i} initial={{ opacity: 0, x: isEven ? -30 : 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 1.2 }} className={`flex items-center w-full relative ${isEven ? 'flex-row' : 'flex-row-reverse'}`}>
                          <div className="w-[45%]">
                            <div className={`overflow-hidden bg-white/60 rounded-2xl border border-amber-50 backdrop-blur-sm shadow-lg ${isEven ? 'text-right' : 'text-left'}`}>

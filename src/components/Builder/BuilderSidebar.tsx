...
export function BuilderSidebar({ invitation, onInvitationChange, activeTab }: any) {
  ...
  const uploadFile = async (e: any, field: string) => {
    ...
    try {
      ...
      alert(lang === 'fr' ? "Erreur d'upload" : "Upload error"); 
    } finally { 
      setUploading(false); 
    }
  };
    ...
    <div className="w-full space-y-8 pb-10">
        ...
        <div className="space-y-8">
          ...
          <div>
            <label className="text-[10px] font-black uppercase text-gray-400 mb-4 block ml-1">
              {lang === 'vi' ? 'Kiểu chữ' : lang === 'en' ? 'Font Style' : "Style d'écriture"}
            </label>
      ...
      {activeTab === 'media' && (
        <div className="space-y-8">
            ...
            {invitation.main_photo_url && (
              <div className="p-5 mt-4 bg-gray-50 rounded-[2rem] space-y-5 border border-gray-100">
                <div className="flex items-center gap-2 text-[9px] font-black text-gray-400 uppercase tracking-widest">
                  <Move size={12}/> {lang === 'vi' ? 'ĐIỀU CHỈNH' : lang === 'en' ? 'ADJUST' : 'AJUSTER'}
                </div>
          ...
          <div>
             <label className="text-[10px] font-black uppercase text-gray-400 mb-4 block ml-1">{t.music_label}</label>
             <div className="p-6 bg-amber-50/50 rounded-[2.5rem] border border-amber-100">
                <label className="flex flex-col items-center justify-center h-24 bg-white rounded-2xl border-2 border-dashed border-amber-200 cursor-pointer hover:border-amber-400 transition-colors">
                   <Music className="w-6 h-6 text-amber-300"/><span className="text-[9px] font-black text-amber-400 uppercase mt-2">{t.upload_music}</span>

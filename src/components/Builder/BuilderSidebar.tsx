...
export function BuilderSidebar({ invitation, onInvitationChange, activeTab }: any) {
 ...
 const handleThemeClick = (themeId: string, isPremium: boolean) => {
  if (isPremium && invitation.plan_type !== 'PREMIUM') {
   alert(lang === 'vi' ? "Vui lòng nâng cấp để sử dụng chủ đề này!" : lang === 'en' ? "Please upgrade to use this theme!" : "Passez au Premium pour débloquer ce thème !");
   return;
  }
 ...
 const handleFontClick = (fontFamily: string, isPremium: boolean) => {
  if (isPremium && invitation.plan_type !== 'PREMIUM') {
   alert(lang === 'vi' ? "Vui lòng nâng cấp để sử dụng font chữ này!" : lang === 'en' ? "Please upgrade to use this font!" : "Passez au Premium pour débloquer cette calligraphie !");
   return;
  }
 ...
 const EVENT_TYPES = [
  { id: 'wedding', name: lang === 'vi' ? 'Đám cưới' : lang === 'en' ? 'Wedding' : 'Mariage', icon: Heart, premium: false },
  { id: 'birthday', name: lang === 'vi' ? 'Sinh nhật' : lang === 'en' ? 'Birthday' : 'Anniversaire', icon: PartyPopper, premium: false },
  { id: 'party', name: lang === 'vi' ? 'Bữa tiệc' : lang === 'en' ? 'Party' : 'Fête', icon: Sparkles, premium: true },
  { id: 'baptism', name: lang === 'vi' ? 'Rửa tội' : lang === 'en' ? 'Baptism' : 'Baptême', icon: Baby, premium: true },
  ...
  <div className="w-full space-y-8 pb-10">
     ...
     <div className="space-y-4">
      <div className="flex items-center justify-between ml-1">
       <label className="text-[10px] font-black uppercase text-gray-400">
        {lang === 'vi' ? 'Chương trình vàng' : lang === 'en' ? 'Golden Program' : "Programmes d'or"}
       </label>
        ...
        <div key={index} className="space-y-2 bg-white p-3 rounded-2xl border border-gray-100 shadow-sm">
         ...
         <label className="flex items-center gap-3 p-2 bg-gray-50 rounded-xl cursor-pointer">
          ...
          <span className="text-[10px] font-bold text-gray-500 uppercase">
           {step.image_url 
            ? (lang === 'vi' ? 'Sửa ảnh' : lang === 'en' ? 'Modify photo' : 'Modifier photo') 
   ...
   {activeTab === 'media' && (
    <div className="space-y-8">
     <div className="space-y-4">
      <label className="text-[10px] font-black uppercase text-gray-400 ml-1">
       {lang === 'vi' ? 'Ảnh & Nhạc' : lang === 'en' ? 'Photos & Music' : 'Photos & Musique'}
      </label>
      <div className="grid grid-cols-2 gap-4">
       <label className="flex flex-col items-center justify-center aspect-square bg-gray-50 rounded-[2rem] border-2 border-dashed border-gray-200 cursor-pointer overflow-hidden relative">
        ...
        <span className="absolute inset-0 flex items-center justify-center text-[10px] font-bold text-gray-600 uppercase bg-white/40 text-center px-2">
         {lang === 'vi' ? 'Ảnh bắt đầu' : lang === 'en' ? 'Start photo' : 'Photo de début'}
        </span>
       ...
       <label className="flex flex-col items-center justify-center aspect-square bg-gray-50 rounded-[2rem] border-2 border-dashed border-gray-200 cursor-pointer overflow-hidden relative">
        ...
        <span className="absolute inset-0 flex items-center justify-center text-[10px] font-bold text-gray-600 uppercase bg-white/40 text-center px-2 flex flex-col items-center gap-1">
         {lang === 'vi' ? 'Ảnh kết thúc' : lang === 'en' ? 'End photo' : 'Photo de fin'}
         {invitation.plan_type !== 'PREMIUM' && <Lock size={16} className="text-gray-400" />}
        </span>
        <input type="file" className="hidden" accept="image/*" onChange={(e) => {
         if(invitation.plan_type !== 'PREMIUM') return alert("Premium !");
        ...
        <>
         <label className="flex flex-col items-center justify-center aspect-square bg-gray-50 rounded-[2rem] border-2 border-dashed border-amber-200 cursor-pointer overflow-hidden relative">
          ...
          <span className="absolute inset-0 flex items-center justify-center text-[9px] font-black text-amber-900 uppercase bg-white/40 text-center px-2">
           {lang === 'vi' ? 'Phim 02' : lang === 'en' ? 'Vue 02' : 'Vue 02'}
          </span>
         ...
         <label className="flex flex-col items-center justify-center aspect-square bg-gray-50 rounded-[2rem] border-2 border-dashed border-amber-200 cursor-pointer overflow-hidden relative">
          ...
          <span className="absolute inset-0 flex items-center justify-center text-[9px] font-black text-amber-900 uppercase bg-white/40 text-center px-2">
           {lang === 'vi' ? 'Phim 03' : lang === 'en' ? 'Vue 03' : 'Vue 03'}
          </span>
       ...
       <label className="col-span-2 flex items-center gap-3 p-4 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200 cursor-pointer transition-colors hover:bg-gray-100">
        ...
        <span className="text-[10px] font-bold text-gray-500 uppercase truncate">
         {invitation.music_url 
          ? (lang === 'vi' ? 'Đã tải nhạc ✓' : lang === 'en' ? 'Music loaded ✓' : 'Musique chargée ✓') 
          : t.upload_music}
        </span>
      ...
      <div className="bg-amber-50/50 p-6 rounded-[2rem] border border-amber-100 space-y-4">
       <div className="flex flex-wrap gap-2">
        <button onClick={() => setSelectedPhotoKey('main_photo_url')} className={`px-3 py-1 rounded-full text-[9px] font-black uppercase transition-all ${selectedPhotoKey === 'main_photo_url' ? 'bg-amber-500 text-white shadow-sm' : 'bg-white text-amber-800 border border-amber-200'}`}>
         {lang === 'vi' ? 'Đầu' : lang === 'en' ? 'Start' : 'Début'}
        </button>
        {invitation.end_photo_url && <button onClick={() => setSelectedPhotoKey('end_photo_url')} className={`px-3 py-1 rounded-full text-[9px] font-black uppercase transition-all ${selectedPhotoKey === 'end_photo_url' ? 'bg-amber-500 text-white shadow-sm' : 'bg-white text-amber-800 border border-amber-200'}`}>
         {lang === 'vi' ? 'Cuối' : lang === 'en' ? 'End' : 'Fin'}
        </button>}
       ...
       <span className="text-[10px] font-black uppercase text-amber-800 tracking-wider flex items-center gap-2">
        <Move size={12}/> {lang === 'vi' ? 'Điều chỉnh ảnh đã chọn' : lang === 'en' ? 'Adjust selected photo' : 'Ajuster la photo selectionnée'}
       </span>
       <div 
    ...
    <div className="space-y-8">
     <div>
      <label className="text-[10px] font-black uppercase text-gray-400 mb-4 block ml-1">
       {lang === 'vi' ? 'Kiểu hiệu ứng' : lang === 'en' ? 'Animation style' : "Style d'animation"}
      </label>
      <div className="grid grid-cols-2 gap-3">
        <button onClick={() => onInvitationChange({...invitation, opening_type: 'vinyl'})}
         className={`flex items-center gap-3 p-4 rounded-2xl border-2 transition-all relative ${invitation.opening_type === 'vinyl' ? 'border-amber-400 bg-amber-50' : 'bg-white border-transparent'}`}>
         <Disc size={18} className={invitation.opening_type === 'vinyl' ? 'text-amber-500' : 'text-gray-400'} />
         <span className="text-[10px] font-bold uppercase">{lang === 'vi' ? 'Đĩa than' : lang === 'en' ? 'Vinyl' : 'Vinyle'}</span>
        </button>
        <button onClick={() => onInvitationChange({...invitation, opening_type: 'filmstrip'})}
         className={`flex items-center gap-3 p-4 rounded-2xl border-2 transition-all relative ${invitation.opening_type === 'filmstrip' ? 'border-amber-400 bg-amber-50' : 'bg-white border-transparent'}`}>
         <Film size={18} className={invitation.opening_type === 'filmstrip' ? 'text-amber-500' : 'text-gray-400'} />
         <span className="text-[10px] font-bold uppercase">{lang === 'vi' ? 'Cuộn phim' : lang === 'en' ? 'Filmstrip' : 'Pellicule'}</span>
        </button>
      </div>
     </div>
     ...
     <div>
      <label className="text-[10px] font-black uppercase text-gray-400 mb-4 block ml-1">
       {t.theme_label} ({lang === 'vi' ? 'Mưa Emojis' : lang === 'en' ? 'Emoji Rain' : "Theme (Pluie d'Emojis)"})
      </label>
      <div className="grid grid-cols-2 gap-3">
     ...
     <div>
      <label className="text-[10px] font-black uppercase text-gray-400 mb-4 block ml-1">
       {lang === 'vi' ? 'Chất liệu' : lang === 'en' ? 'Texture' : 'Texture'}
      </label>
     ...
     <div>
      <label className="text-[10px] font-black uppercase text-gray-400 mb-4 block ml-1">
       {lang === 'vi' ? 'Màu phong bì' : lang === 'en' ? 'Envelope Color' : 'Couleur de l\'enveloppe'}
      </label>
     ...
     <div>
      <label className="text-[10px] font-black uppercase text-amber-600 mb-4 flex items-center gap-2 ml-1">
       <span className="flex items-center gap-2"><Sparkles size={12}/> 
        {lang === 'vi' ? 'MÀU SATIN' : lang === 'en' ? 'SATIN COLORS' : 'COULEUR SATIN'}
       </span>
      </label>
     ...
     <div>
      <label className="text-[10px] font-black uppercase text-gray-400 mb-4 block ml-1">
       {lang === 'vi' ? 'Kiểu chữ' : lang === 'en' ? 'Writing Style' : "Style d'écriture"}
      </label>

...
export function GuestView({ invitation }: any) {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center overflow-hidden touch-none" style={{ backgroundColor: invitation.envelope_color || '#F3F4F6' }}>
      ...
      {/* CONTENEUR GLOBAL FIXE (Pas de rendu conditionnel ici) */}
      <div className="relative w-full h-full flex items-center justify-center">
        
        {/* PHASE 2 (Toujours là, cachée par l'enveloppe en Z-index) */}
        <div 
          className="relative w-full max-w-[400px] h-full grid place-items-center transition-opacity duration-500"
          style={{ 
            opacity: view === 'envelope' ? 1 : 0, 
            pointerEvents: view === 'envelope' ? 'auto' : 'none',
            visibility: view === 'envelope' ? 'visible' : 'hidden'
          }}
        >
            {/* DISQUE */}
            <motion.div 
            ...
            >
              <div className="text-center pt-10">
                {/* TITRE STABILISÉ */}
                <h2 className="text-3xl font-black uppercase gold-shimmer" style={{ fontFamily: invitation.font_style, transform: 'translateZ(0)' }}>
                  {invitation.title}
                </h2>
            ...
            {/* ENVELOPPE (PHASE 1) */}
            <motion.div 
              animate={isOpened ? { y: '-110%', opacity: 0 } : { y: 0, opacity: 1 }}
              transition={{ duration: 0.8, ease: [0.6, 0.01, -0.05, 0.95] }}
              className="absolute inset-0 z-50 flex flex-col items-center justify-center"
              style={{ backgroundColor: invitation.envelope_color, pointerEvents: isOpened ? 'none' : 'auto' }}
            >
              <motion.button 
        ...
        <AnimatePresence>
          {view === 'content' && (
            <motion.div key="content" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className={`fixed inset-0 z-[100] flex flex-col overflow-y-auto overflow-x-hidden touch-pan-y w-full ${getPaperClass()}`}>
              ...
              <div className="flex-1 px-8 py-16 max-w-2xl mx-auto w-full space-y-24">
                <div className="text-center space-y-8">
                  <h2 className="text-sm font-black uppercase tracking-[0.5em] gold-shimmer" style={{ fontFamily: invitation.font_style }}>{invitation.title}</h2>
                  ...
                  <div className="flex flex-col items-center gap-6 pt-6">
                      <div className="flex items-center gap-5 bg-white/80 p-1.5 pr-6 rounded-full shadow-lg border border-amber-100">
                        ...
                        <span className="text-sm font-black uppercase tracking-widest">{new Date(invitation.event_date).toLocaleDateString('fr-FR', {day:'numeric', month:'long', year:'numeric'})}</span>
                        <button onClick={addToCalendar} className="ml-3 p-2 bg-gray-100 rounded-full hover:bg-amber-100 transition-colors"><Plus size={18} className="text-amber-600" /></button>
                      </div>
                  </div>
                </div>

                {/* RSVP FORM */}
                <div className="bg-white/90 backdrop-blur-2xl rounded-[5rem] p-12 shadow-2xl border border-white">
                  {!isSubmitted ? (
                    <form onSubmit={handleRSVP} className="space-y-10">
                      ...
                      <div className="space-y-5">
                        {guests.map((guest, i) => (
                          <div key={i} className="grid grid-cols-2 gap-4">
                            <input required placeholder="Prénom" className="bg-white/50 border-none h-16 px-6 rounded-2xl text-sm outline-none" value={guest.firstName} onChange={e => {
                              const newGuests = [...guests];
                              newGuests[i].firstName = e.target.value;
                              setGuests(newGuests);
                            }} />
                            <input required placeholder="Nom" className="bg-white/50 border-none h-16 px-6 rounded-2xl text-sm outline-none" value={guest.lastName} onChange={e => {
                              const newGuests = [...guests];
                      ...
                      <button disabled={isSubmitting} className="w-full h-20 bg-gray-900 text-white rounded-[2rem] font-black uppercase tracking-[0.4em] shadow-2xl flex items-center justify-center gap-4">

...
export function GuestView({ invitation }: any) {
  ...
  const showPremiumMidSection = isPremium && (premiumMidTitle || premiumMidText || premiumMidPhotoUrl);
  const showPremiumFinalSection = isPremium && (premiumFinalTitle || premiumFinalText || premiumFinalPhotoUrl);
        ...
        {view === 'envelope' ? (
          <motion.div key="env" className="relative w-full h-full flex items-center justify-center" style={{ perspective: '1200px' }}>
            ...
            <div className="absolute inset-0 z-50 overflow-hidden" style={{ perspective: '2000px', pointerEvents: isOpened ? 'none' : 'auto' }}>
              <AnimatePresence>
                {(!isOpened || containerOpen === 'metal_door') && (
                    ...
                    <AnimatePresence>
                      {!isCodeFading && (
                        <motion.div
                          key="visual-trigger"
                          initial={{ opacity: 1 }}
                          exit={containerOpen === 'metal_door' ? {} : { opacity: 0, transition: { duration: 0.4, ease: 'easeInOut' } }}

...
export function GuestView({ invitation }: any) {
  ...
  const handleOpen = () => {
    setShowTrigger(false); // Suppression immédiate du visuel
    setIsOpened(true); // Lancement de l'animation de l'enveloppe
            ...
            <div className="absolute inset-0 z-50 overflow-hidden" style={{ perspective: '2000px', pointerEvents: isOpened ? 'none' : 'auto' }}>
              <AnimatePresence>
                {!isOpened && (
                  ...
                  >
                    {/* VISUEL INTERACTIF : showTrigger garantit une suppression instantanée du DOM */}
                    {showTrigger && (

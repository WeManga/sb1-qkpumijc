                  ...
                  <div className="relative flex flex-col items-center">
                    <div className="relative space-y-12 w-full">
                      {(invitation.event_program || []).map((step: any, i: number) => {
                        const isEven = i % 2 === 0;
                        const isLast = i === (invitation.event_program.length - 1);
                        return (
                          <motion.div key={i} initial={{ opacity: 0, x: isEven ? -30 : 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 1.2 }} className={`flex items-center w-full relative ${isEven ? 'flex-row' : 'flex-row-reverse'}`}>
                            {/* Segment du chemin (Ligne verticale entre les points) */}
                            {!isLast && (
                              <div className="absolute left-1/2 -translate-x-1/2 top-1/2 h-full w-[2px] z-0">
                                <motion.div 
                                  initial={{ scaleY: 0 }} 
                                  whileInView={{ scaleY: 1 }} 
                                  viewport={{ once: true, margin: "-100px" }} 
                                  transition={{ duration: 0.8, ease: "easeOut" }} 
                                  className="w-full h-full bg-gradient-to-b from-amber-500 to-amber-200 origin-top"
                                />
                              </div>
                            )}
                            ...
                            <div className="w-[45%]">
                              <div className={`overflow-hidden bg-white/80 rounded-2xl border border-amber-50 shadow-lg ${isEven ? 'text-right' : 'text-left'}`}>
                                ...
                                <div className="p-4">
                                  <div className={`text-[9px] font-black text-amber-600 mb-1 flex items-center gap-1 ${isEven ? 'justify-end' : 'justify-start'}`}><Clock size={8}/> {step.time}</div>
                            ...
                            <div className="w-[10%] flex justify-center relative z-10">
                              <motion.div 
                                ...
                                className="w-3 h-3 bg-amber-500 rounded-full ring-4 ring-white shadow-sm" 
                              />
                            </div>
